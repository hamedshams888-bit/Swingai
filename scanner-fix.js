  ...
  function wire(btn){
    ...
    btn.addEventListener('click',()=>{
      ...
      observer=new MutationObserver(()=>{
        const cards=[...document.querySelectorAll('.global-signal,.signal,.opportunity-card')].filter(visible);
        const loading=[...document.querySelectorAll('.loading')].some(visible);
        if(cards.length&&!loading)finish(btn,true,'✅ اسکن کامل شد — نتایج آماده است');
        else if(Date.now()-started>45000)finish(btn,false,'⚠️ اسکن بیش از ۴۵ ثانیه طول کشید؛ دوباره تلاش کن');
      });
